...
impl<'a, L: EasyLocation<'a>> EasyReport for EasyReporting<'a, L> {
    fn emit<E: Display>(&self, diag: impl Into<Diagnostic<E>>) {
        ...
        term::emit_to_write_style(&mut self.writer.lock(), &self.config, self.source, &diag)
...
impl<'a, L: EasyLocation<'a>> EasyReporting<'a, L> {
    ...
    pub fn with_config(source: &'a L, config: Config) -> Self {
        ...
        Self {
            ...
            config,
            source,
            errors: AtomicU32::default(),
            warnings: AtomicU32::default(),
        }
    }
    ...
    pub fn emit_status(&self) -> EasyReportingStatus {
        ...
        match self.errors.load(Ordering::SeqCst) {
            0 => {
                ...
                if warns == 0 {
                    EasyReportingStatus::Faultless
                } else {
                    EasyReportingStatus::Warnings(warns)
                }
            }
            1 => {
                term::emit_to_write_style(
...
impl<E: Display> Diagnostic<E> {
    ...
    pub fn with_labeled_note(mut self, label: impl AsRef<str>, note: impl Into<String>) -> Self {
        ...
        self.notes
            .push(format!("\x1B[1m{}\x1B[0m: {}", label.as_ref(), note.into()));
        self
    }
    ...
    pub fn with_primary_label(
        ...
        self.labels
            .push((diagnostic::LabelStyle::Primary, range, msg.into()));
        self
    }
    ...
    pub fn with_secondary_label(
        ...
        assert![!range.is_empty(), "invalid (empty) location"];
        self.labels
            .push((diagnostic::LabelStyle::Secondary, range, msg.into()));
        self
    }
    ...
    pub fn to_diagnostic<'a, L: EasyLocation<'a>>(
        self,
        src: &'a L,
    ) -> diagnostic::Diagnostic<<L as Files<'a>>::FileId> {
    ...
    #[inline]
    pub fn report<R: EasyReport>(self, report: &R) {
        report.emit(self)
    }
}
...
impl<E: Display> Debug for Diagnostic<E> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        self.notes
            .iter()
            .try_for_each(|note| writeln!(f, "   {}", note))
    }
}
